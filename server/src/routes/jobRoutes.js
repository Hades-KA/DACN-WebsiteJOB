...
const { Application, Job, CV, User, Score } = require('../models');

// DÙNG scoreService LÀ TRUNG TÂM GỌI AI
  ...
  async (req, res) => {
    try {
      ...
      const u = await User.findByPk(candidateId, {
        attributes: [
          'id','name','email','phone',
          'position','location','about','skills','experience','education','avatar',
          'level','workType','degree','industry','jobCategory',
          'experienceBand','expectedSalary','birthdate','address','gender','maritalStatus',
          'careerGoals','cvUrl','cvName'
        ],
      });
      ...
      const snapshot = u ? {
        ...
        experienceBand: u.experienceBand,
      ...
      console.log(`[Apply] Application ${application.id} created, AI scoring enqueued`);
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      if (!job) {
        return res.status(404).json({ message: 'Job không tồn tại hoặc bạn không có quyền' });
      }

      // 2) Lưu JD + must/nice-have (dưới dạng chuỗi JSON) + tăng jdVersion
      ...
      console.log(`[JD Update] Re-scoring ${apps.length} applications for job ${jobId}...`);
...
/* ================= RESCORE TẤT CẢ ỨNG VIÊN CỦA 1 JOB (ĐÃ SỬA AN TOÀN HƠN) ================= */
router.post(
  ...
  async (req, res) => {
    try {
      ...
      console.log(`[Rescore All] jobId=${jobId}, employerId=${employerId}, onlyMissing=${onlyMissing}`);
      ...
      // Kiểm tra xem model Score có tồn tại không
      const hasScoreModel = !!Score;
      const orderField = hasScoreModel && Score.rawAttributes?.generatedAt ? 'generatedAt' : 'createdAt';
      const cutoff = staleMinutes ? new Date(Date.now() - staleMinutes * 60 * 1000) : null;
      ...
      for (const a of apps) {
        ...
        if (onlyMissing && hasScoreModel) {
          try {
            ...
            console.warn(`[Rescore All] Error checking score for app ${a.id}:`, scoreErr.message);
            // Nếu lỗi khi check score, vẫn cho phép rescore
            should = true;
          }
        }
